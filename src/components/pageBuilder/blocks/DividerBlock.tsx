...
function DividerBlockComponent({ props }: DividerBlockProps) {
  ...
  const dividerStyle = {
    ...
    height: `${height}px`,
    border: 'none',
    borderTop: `${height}px ${style} ${effectiveColor}`,
    backgroundColor: style === 'solid' ? effectiveColor : 'transparent',
    backgroundImage: style === 'dashed' ? `repeating-linear-gradient(90deg, ${effectiveColor} 0, ${effectiveColor} 10px, transparent 10px, transparent 20px)` :
      style === 'dotted' ? `radial-gradient(circle, ${effectiveColor} 1px, transparent 1px)` : 'none',
